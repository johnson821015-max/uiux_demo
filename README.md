...
打開 `index.html`，**頂端列是風格分頁**：靛紫 SaaS ／ 暖石墨工作台 ／ 工作台儀表。切換分頁，下方所有元件立刻換一套風格。

分頁下方的「調整數值細項」面板可以即時改色票、圓角、字級、陰影；「＋ 新增風格」會複製當前風格讓你另外調一套；「匯出 CSS」把調好的值吐成一段 CSS。

> 面板的調整只暫存在瀏覽器。**要永久保存，得把匯出的 CSS 貼回 `index.html`。**

所有風格都由 `<style>` 開頭的 **【DESIGN TOKENS】** 區塊控制。每套風格一組 `:root[data-style="…"]`，深色模式是每套風格底下的 `[data-theme="dark"]` 覆寫——風格與明暗是兩個獨立的維度。可調的變數有：
...
**改風格 = 只改這些變數**，全部元件自動跟著變。

## 設計準則

Tokens 的取值依循「去 AI 感」設計方法——一個有觀點的重點色、選過的暖中性色、髮絲線取代浮空陰影、圓角收斂、數字等寬對齊。

方法全文與審查檢查清單見 [`design-reference/`](design-reference/)，日常開發規則見 [`CLAUDE.md`](CLAUDE.md)。
...
- **卡片**：基本、圖片、橫式、統計卡、價格卡
- **電商版面**：促銷橫幅＋倒數計時、商品列表頁（篩選側欄/排序/商品卡含快速加購）、商品詳情頁（縮圖圖庫/顏色版本規格選擇）、閃購卡、優惠券、評價區（分數分佈/評論卡）、購物車頁（品項/優惠碼/結帳摘要）
- **導覽**：Navbar、Tabs、分段控制、麵包屑、分頁、步驟指示
- **資料**：表格、列表、樹狀列（展開/收合/選取）、待辦清單、分組黏性標題列表、排行榜、描述列表（規格表）、Badge、Chip、頭像、Tooltip
...
- **手機 App（7 種畫面）**：首頁、登入頁、聊天室（訊息氣泡/輸入中指示/可實際輸入）、個人頁、電商（限時動態圈/商品卡/購物車徽章）、設定頁、通知中心；含 App Bar、底部導覽、FAB、Bottom Sheet
- **第三方整合**：地圖（圖釘/定位/縮放/地點卡片）、金流付款（訂單摘要/信用卡表單自動格式化/Apple Pay/Google Pay/LINE Pay）、社群分享按鈕、影片嵌入播放器、客服聊天小工具、Cookie 同意橫幅
- **版面**：Hero、特色區、側欄後台版型、Footer

## 使用流程

1. 用瀏覽器打開 `index.html`，用頂端分頁挑一套風格當起點，右上角可切換深色模式
2. 展開「調整數值細項」邊調邊看；要另立一套就按「＋ 新增風格」
3. 按「匯出 CSS」，把結果貼回 `index.html` 的 DESIGN TOKENS 區永久保存
4. 把這個檔案丟給 AI，用類似下面的提示詞
